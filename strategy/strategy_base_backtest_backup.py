...
class StrategyBaseBacktestStock(object):
        ...
        for order in list(self.context.active_limit_orders.values()):
            ...
            if long_cross or short_cross:
                ...
                if order.offset == Offset.OPEN:
                    frozen_volume = order.order_volume
                else:
                    frozen_volume = 0
            ...
            # elif not long_cross and not short_cross:
                ...
                order_new.comments = order.comments + '_追单'
                ...
                if order_new.offset == Offset.OPEN:
                    cur_str = '买入'
                    frozen_volume = order_new.order_volume
                else:
                    cur_str = '卖出'
                    frozen_volume = 0
    ...
    def handle_trade_(self, event_trade):
        ...
        1）更新当前交易的成交信息、交易成本
        2）并保存入 current_xx_data 中，随后 current_xxx_data 的最新值存入以当前 bar 的时间戳为 key 的 bar_xxx_data_dict 中
        """
        ...
        self.context.current_position_data.position_value = \
            self.context.current_trade_data.price * self.context.current_position_data.volume
