...
class StrategyBase(object):
    ...
    def run_strategy(self):
        ...
        while True:
            ...
            try:
                if self.run_mode == RunMode_BACKTESTING:  # 回测模式下，市场数据通过生成器推送过来
                    self.timestamp = next(bmi_iter)
                else:
                    # live模式下，市场数据通过api订阅后推送过来
                    pass
            except Exception:
                print('策略运行完成')
                break
                ...
                sleep(1)
                # print("{0} {1}".format(self.datetime, cur_event.data))
    ...
    def cross_stop_order(self, event_market, cur_mkt_data):
        ...
        for stop_order in list(self.context.active_stop_orders.values()):
            ...
            order = OrderData(symbol=stop_order.symbol,
                              ...
                              traded_volume=stop_order.order_volume,
    ...
    def handle_order(self, event_order):
        ...
        print('handle_order() method @ {0}'.format(event_order.data.order_datetime))
        ...
        self.context.current_order_data.traded_volume = 0
        ...
        if event_order.data.offset == 'open':
            ...
            trade_balance = self.context.current_order_data.order_volume * \
                            self.context.current_order_data.price * contract_params['margin']
            if trade_balance / 0.90 > self.context.bar_account_data_list[event_order.data.account].available:
                ...
                if self.context.bar_position_data_list:
                    for position_data in self.context.bar_position_data_list:
                        ...
                        for account_data in self.context.bar_account_data_list:
                            if account_data.account_id == self.context.current_order_data.session_id:
    ...
    def handle_risk(self, event_order):
        """从队列中获取到订单事件，进行前置风控的审核，根据注册时的顺序，一定发生在 handle_order() 之后：
        要交易的合约是否在黑名单上
    ...
    def handle_trade(self, event_trade):
        ...
        self.context.current_trade_data.frozen += self.context.current_order_data.traded_volume
        ...
        self.context.current_position_data.frozen += self.context.current_trade_data.traded_volume
